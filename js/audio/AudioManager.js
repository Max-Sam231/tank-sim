...
export class AudioManager {
  constructor() {
    ...
    this.prevState = {
      ...
      combined: false,
      heating: false,
      waterAntifreeze: false // ДОБАВЛЕНО
    };
    ...
    this.sounds.starter.volume = 0.5;
  ...
  updateEngineSound(rpm, isRunning) {
    if (!this.isInitialized) return;
    ...
    if (isRunning) {
      ...
      engine.playbackRate = Math.max(0.8, Math.min(1.8, rate));
      engine.volume = 0.4 + ((rpm - 900) / 2500) * 0.3;
    } else {
      if (!engine.paused) {
        engine.pause();
  ...
  onStateChange(state) {
    ...
    if (state.airBleedValve && !this.prevState.airBleedValve) {
      ...
      if (hasPressure && isTankOpen) {
        ...
        console.warn("️ Кран открыт, но нет давления или баллоны закрыты. Звук воздуха пропущен.");
      }
    ...
    // ==========================================
    // ТУМБЛЕРЫ И РЫЧАЖКИ (звук switch - более весомый)
    // ==========================================
    ...
    // 26. Вода/Антифриз (ДОБАВЛЕНО)
    if (state.waterAntifreeze !== this.prevState.waterAntifreeze) {
      console.log("💧 Вода/Антифриз: " + (state.waterAntifreeze ? "ВКЛ" : "ВЫКЛ"));
      this.play('switch');
    }

    // 27. Обновление непрерывных звуков (только двигатель)
    this.updateEngineSound(state.engine_rpm, state.engineRunning);

    // 28. ОБНОВЛЕНИЕ prevState (СТРОГО В КОНЦЕ)
    this.prevState = {
      horn: state.horn,
      ...
      waterAntifreeze: state.waterAntifreeze // ДОБАВЛЕНО
    };
  }
}